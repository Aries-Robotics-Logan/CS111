...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    if num_rolls == 0:
        ret = picky_piggy(opponent_score)
    else:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def f(score0, score1):
        if who == 0:
            rh = running_high
            num = score0 - last_score
            if num > rh:
                print(f"{num} point(s)! That's a record gain for Player 0!")
                rh = num
            return announce_highest(who, score0, rh)
        if who == 1:
            rh = running_high
            num = score1 - last_score
            if num > rh:
                print(f"{num} point(s)! That's a record gain for Player 1!")
                rh = num
            return announce_highest(who, score1, rh)
    return f
    # END PROBLEM 7
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    opp = picky_piggy(opponent_score)
    if opp >= cutoff:
        return 0
    else:
        return num_rolls
