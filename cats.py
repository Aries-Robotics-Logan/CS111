...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    counter = -1
    while k > -1 and counter + 1 < len(paragraphs):
        counter += 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def new_function(paragraph):
        check = 0
        sentence = split(lower(remove_punctuation(paragraph)))
        for i in topic:
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    multiplication = 60/elapsed
    totleng = len(typed)
    words = totleng / 5
    return words * multiplication
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        list.append(diff_function(typed_word, word, limit))
    smallest = min(list)
    ret = list.index(smallest)
    if smallest > limit:
        return typed_word
    elif typed_word in valid_words:
        return typed_word
    else:
        return valid_words[ret]
    # END PROBLEM 5
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for n in range(len(times_per_player[i])-1):
            new_list.append(times_per_player[i][n+1]-times_per_player[i][n])
...
def fastest_words(match):
    ...
    times = get_times(match) # [[1,2,4],[3,5,6]]  [0][0] [1][0]   [0][1] [1][1]   [0][2] [1][2]
    list = []
    ...
    for i in word_indices:
        ...
        for n in player_indices:
            if fastesttime == 0 or times[n][i] < fastesttime:
                fastesttime = times[n][i]
                correct_index = n
        list[correct_index].append(words[i])
    return list
